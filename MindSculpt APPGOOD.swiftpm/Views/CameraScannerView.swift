...
struct CameraScannerView: View {
    ...
    var body: some View {
        ...
        // FIX: nueva firma de onChange para iOS 17
        .onChange(of: cameraManager.currentColor) { _, newColor in
...
final class CameraManager: NSObject, ObservableObject {
    ...
    private let videoQueue = DispatchQueue(
        label: "com.olmos.MindSculpt.videoQueue",
        qos: .userInitiated
    )

    /// Configura los inputs/outputs en main, y arranca la sesión en background.
    func startSession() {
        ...
        do {
            ...
            // 👇 Forzamos BGRA para que el sampling sea predecible y seguro
            ...
            // 👇 startRunning() es BLOQUEANTE — debe correr fuera de main
...
extension CameraManager: AVCaptureVideoDataOutputSampleBufferDelegate {
    nonisolated func captureOutput(
        ...
        guard let color = Self.sampleCenterColor(pixelBuffer: pixelBuffer) else { return }
    ...
    /// Lee un color promedio del centro del frame directamente del CVPixelBuffer.
    /// SIN crear CIImage ni CGImage (que causaba presión de memoria y crashes).
    /// Asume formato BGRA (forzado en videoSettings).
    nonisolated private static func sampleCenterColor(
        ...
        guard let base = CVPixelBufferGetBaseAddress(pixelBuffer),
              width  > radius * 2,
              height > radius * 2 else { return nil }
        ...
        return Color(
            red:   Double(rSum) / Double(count) / 255.0,
            green: Double(gSum) / Double(count) / 255.0,
            blue:  Double(bSum) / Double(count) / 255.0
        )
    }
}
...
struct CameraPreviewView: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: UIView, context: Context) {
        // ✅ Crear el preview layer la primera vez que haya sesión disponible.
        // updateUIView se vuelve a llamar cuando isSessionReady cambia,
        // porque cameraManager es @ObservedObject.
        if context.coordinator.previewLayer == nil,
           let session = cameraManager.getCaptureSession() {
        ...
        // Actualizar el frame en rotaciones / cambios de bounds, sin animación.
        CATransaction.begin()
